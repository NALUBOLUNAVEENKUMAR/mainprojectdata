...
PreparedStatement ps=con.prepareStatement("select * from userreques,upload,reg where reg.id=userreques.userid and userreques.fid=upload.fid and userreques.userid='"+session.getAttribute("uid")+"' ");
...
while(rs.next()){
	...
	<%=rs.getString(8)	 %>
		...
		<td>
	<%
	PreparedStatement ps1=con.prepareStatement("select * from dataproviders where dpid='"+rs.getString(3)+"'");
...
<input type="hidden" name="fid" value="<%=rs.getString(4)%>">
